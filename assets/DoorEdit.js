...
import { getDataDoor } from "../views/Utills"
    ...
    componentDidMount() {
        getDataDoor((data) => {
            ...
            this.state.data.map((item, index) => {
                ...
                this.setState({ ResTotald: total })
            })
            console.log("ads", this.state.ResTotald)
            AsyncStorage.setItem('doors', JSON.stringify(this.state.ResTotald));
        })
        ...
        return (
            ...
            renderItem={({item}) => { */}
                    ...
                    {data.map((item, index) => {
                        ...
                        return <TouchableOpacity onPress={() => this.props.navigation.navigate("Doors", { saveIndex: index })}>
                                ...
                                <View style={styles.eventContent}>
                                    {/* {item.widthd > 0 && */}
                                        <View>
                                            ...
                                            <Text style={styles.userName}>Thickness D42      {item.finalThicknessd}</Text>
                                            <Text style={styles.userName}>Material D42       {item.finalMateriald}</Text>
                                            <Text style={styles.userName}>Width D42          {item.widthd}</Text>
                                            <Text style={styles.userName}>Height D42         {item.heightd}</Text>
                                            <Text style={styles.userName}>Thickness D50A     {item.finalThickness2d}</Text>
                                            <Text style={styles.userName}>Material D50A      {item.finalMaterial2d}</Text>
                                            <Text style={styles.userName}>Width D50A         {item.width2d}</Text>
                                            <Text style={styles.userName}>Height D50A        {item.height2d}</Text>
                                            <Text style={{ textAlign: "center", fontSize: 20 }}>Result        {item.singleResultd}</Text>
                                        </View>
                                    {/* } */}
                                    <View>
                                        {item.width3 > 0 &&
                                    ...
                                    <View>
                                        {item.width5 > 0 &&
                                            <View>
                                                <Text style={styles.userName}>Thickness D50      {item.finalThickness5}</Text>
                                                <Text style={styles.userName}>Material D50       {item.finalMaterial5}</Text>
                                                <Text style={styles.userName}>Width D50          {item.width5}</Text>
                                                <Text style={styles.userName}>Height D50         {item.height5}</Text>
                                                <Text style={styles.userName}>Thickness D54A     {item.finalThickness6}</Text>
                                                <Text style={styles.userName}>Material D54A      {item.finalMaterial6}</Text>
                                                <Text style={styles.userName}>Width D54A         {item.width6}</Text>
                                                <Text style={styles.userName}>Height D54A        {item.height6}</Text>
                                                <Text style={styles.userName}>Thickness D46     {item.finalThickness7}</Text>
                                                <Text style={styles.userName}>Material D46      {item.finalMaterial7}</Text>
                                                <Text style={styles.userName}>Width D46         {item.width7}</Text>
                                                <Text style={styles.userName}>Height D46        {item.height7}</Text>
                                                <Text style={styles.userName}>Result        {item.singleResult}</Text>
                                                <Text style={{ textAlign: "center", fontSize: 20 }}>Result        {item.singleResultd}</Text>
                    ...
                    {/* // }}/> */}
                    <View>
                        <Text style={{ textAlign: "center", fontSize: 20 }}>Total Value For Doors is {this.state.ResTotald}</Text>
                    </View>
                </ScrollView>
            </View>
        );
